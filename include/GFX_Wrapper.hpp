...
public:
    ...
    TFT_GFX() {
        ...
        _panel = new Arduino_AXS15231B(
            _bus, -1 /*RST*/, 0 /*rotation*/, false, 320, 480);
        _gfx = new Arduino_Canvas(320, 480, _panel);
    }

    //   (LovyanGFX: tft.begin() / tft.init()) 
    bool begin() {
    ...
    //  drawString (LovyanGFX   setCursor + print) 
    ...
    //  drawCentreString 
    void drawCentreString(const char* str, int32_t cx, int32_t y,
                          uint8_t /*font*/ = 1) {
        int16_t tw = _textWidth(str);
        _gfx->setCursor(cx - tw / 2, y);
        _gfx->print(str);
    }
    void drawCentreString(const String& str, int32_t cx, int32_t y,
                          uint8_t font = 1) {
        drawCentreString(str.c_str(), cx, y, font);
    }
    ...
    void drawRightString(const char* str, int32_t x, int32_t y,
                         uint8_t /*font*/ = 1) {
        int16_t tw = _textWidth(str);
        _gfx->setCursor(x - tw, y);
        _gfx->print(str);
    }
    void drawRightString(const String& str, int32_t x, int32_t y,
                         uint8_t font = 1) {
    ...
    //  setSwapBytes (LovyanGFX , Arduino_GFX  swap) 
    void setSwapBytes(bool /*swap*/) { /* no-op */ }

    //  pushColors 
    // LovyanGFX: pushColors(data, len, swapBytes)
    //    len  RGB565 
    void pushColors(uint16_t* data, uint32_t len, bool /*swap*/ = true) {
        // Arduino_GFX: draw16bitRGBBitmap 
        // 1  ( )
        _gfx->draw16bitRGBBitmap(_cursorX, _cursorY,
                                  data, len, 1);
    }
    // uint8_t*  (  )
    void pushColors(uint8_t* data, uint32_t len, bool swap = true) {
    ...
    //   AXS15231B I2C  
    // 
    // LovyanGFX: bool tft.getTouch(&x, &y)
