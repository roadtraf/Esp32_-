...
public:
    ...
    void drawString(const char* str, int32_t x, int32_t y) {
        _gfx->setCursor(x, y); _gfx->print(str);
    }
    ...
    void drawCentreString(const char* str, int32_t cx, int32_t y, uint8_t = 1) {
        ...
        _gfx->setCursor(cx - tw / 2, y); _gfx->print(str);
    }
    void drawCentreString(const String& str, int32_t cx, int32_t y, uint8_t font = 1) {
        drawCentreString(str.c_str(), cx, y, font);
    }
    void drawRightString(const char* str, int32_t x, int32_t y, uint8_t = 1) {
        int16_t tw = _textWidth(str);
        _gfx->setCursor(x - tw, y); _gfx->print(str);
    }
    void drawRightString(const String& str, int32_t x, int32_t y, uint8_t font = 1) {
    ...
    void pushColors(uint16_t* data, uint32_t len, bool = true) {
        _gfx->draw16bitRGBBitmap(_cursorX, _cursorY, data, len, 1);
    }
